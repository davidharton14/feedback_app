...
const connection = new Sequelize("train", "root", "", {
  dialect: "mysql"
});
...
const configure = app => {
  ...
  app.use(
    session({
      cookieName: "session",
      secret: 'eg[isfd-8yF9-7w2315dfergergpok123+Ijsli;;termgerdfkhmdkrherhhehwemgro8',
      duration: 7200000,
  ...
  app.post("/professors", async (req, res) => {
    try {
      ...
      if (errors.length === 0) {
        await User.create({ firstName, lastName, email, password, type: 1, token: Math.random().toString(36) });
        res.status(201).send({ message: `Professor ${firstName} ${lastName} was sucessfull created` });
      } else {
  ...
  app.get("/activities", checkLogin, async (req, res) => {
    try {
      ...
      const formattedActivities = await Promise.all(await activities.map(async item => {
    ...
    try {
      ...
      if (!activity) {
        ...
        if (!user) {
          ...
          if (errors.length === 0) {
            user = await User.create({ firstName, lastName, email, type: 2, token: Math.random().toString(36) })
          } else {
            res.status(400).send({ errors });
          }
        }
  ...
  app.post("/feedback", checkLogin, async (req, res) => {
    try {
      ...
      if (!activity) {
        ...
        if (activity.status) {
          await Feedback.create({ emoticon, activityId, userId: req.session.id })
